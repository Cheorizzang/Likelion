...
// NaN은 숫자가 아니기 때문에 NaN을 포함한 그 무엇과도 같지 않다.

// javascript 내부적으로는 비교 x === y(여기서 x와 y는 값)는 true 또는 false를 생성합니다. 이러한 비교는 다음과 같이 수행됩니다.

/* 
Type(x)가 NaN이면 false를
반환합니다. y가 NaN이면 false를 반환합니다. 이것은 알고리즘이 먼저 피연산자 중 하나가 NaN 유형을 확인하기 전에 있는지 확인하고 그렇다면 false를 반환할
것임을 의미합니다. 
*/
console.log(0 / 0); // NaN
...
// ES6에서 추가 도입 (암기)
// Number.isNaN은 기존의 isNaN보다 더 엄격한 버전
