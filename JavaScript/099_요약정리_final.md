...
console
    ...
    // .a()
    // .b()
    // .c() 공백을 병합해서 메서드 체이닝도 가능
    // 실무에서 압축(공백 제거) + 난독화 많이 씀
```
4. 주석
```js
// 한 줄 주석입니다.
...
- Number(숫자)
    ...
    * Math
        * Math.PI
        * Math.max() -> -infinity / 전개 필요
        * Math.min() -> infinity / 전개 필요
        * Math.floor()
        * Math.round()
        * Math.random()
        * Math.abs()
...
- String(문자열)
    * 형태 : 'abcde', "abcde", `abcde ${변수명}` (`: 틸트, 백틱, 템플릿 리터럴)
    * 호출 : 변수명, 변수명[0] (변수명[index], 호출은 할 수 있지만 개별 값 변경 불가)
    * 메서드 : 
        * str.length
        * str.indexOf()
        * str.lastIndexOf() -> 거의 X
        * str.includes()
        * str.slice()
        * str.split()
        * str.substring()
        ...
        * str.repeat()
        ...
        // 여기서 Number는 콜백함수로서, map의 인자로 들어간 것
        ```
...
- Array(배열) : object
    ...
    * Array
    * 메서드 : mutable(원본수정 O) / immutable(원본수정 X)
        * length
        * forEach
        * map // 조건식의 리턴값으로 이루어진 새로운 배열 생성
        * filter
        * push / pop - mutable
        * slice - immutable
        * splice - mutable
...
- switch
    ...
    ```
    ```js
    let result = true ? 1 : 100; // 삼항연산자로 조건문 구현
    ```
...
- while
- do while -> 거의 X
- forEach -> 배열을 단순 순회하는 메서드
- break -> 가까운 반복문 한겹만 탈출
- continue -> 그 다음 반복문으로 넘어감
...
* 클래스 - 붕어빵 찍는 틀, 공장
* 인스턴스 - 붕어빵, 제품
* 클래스는 생성자함수나 클로저를 대체할 수 있음. 반대는 안됨.
```js
class Human {
    constructor() {}
...
number[0] // 8이 나옴 -> 전화번호를 문자열로 다뤄야함
```

## 사용자와 상호작용
* prompt('hello') -> 사용자가 문자열을 입력할 수 있게 함
* comfirm('hello')
* alert('hello')
...
// 성공(resolve)하면 then, 실패(reject)하면 catch로 감
